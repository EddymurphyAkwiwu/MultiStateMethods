...
# v.min   = 1           # Minimum time between screening moments
# v.max   = 4.0           # Maximum time between screening moments
# Tmax    = 2e2        # Maximum number of screening times (this is set to high value; but too high values increase computation time)
# mean.rc = 49
# do.b.XS = F; b.XS = NULL
...
# 
# 
time = state = firstobs= Z.1 = Z.2 = numeric()
copy = NULL
i =1
x = matrix(NA, ncol= 6+length(copy), nrow = 1)
for(i in 1:nrow(dat)){
  time = numeric()
  state = numeric()
  Z.1 = numeric()
  ...
  x.t = cbind(i, time, state, Z.1, Z.2, firstobs)
...
if(p>0) cov.data = data.frame( Z=Z)
